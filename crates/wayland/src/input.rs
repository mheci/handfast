    ...
    impl VirtualInput {
        ...
        pub fn axis(&mut self, vertical: bool, steps: f64) -> Result<()> {
            ...
            let axis = if vertical {
                wl_pointer::Axis::Vertical
            } else {
                wl_pointer::Axis::Horizontal
            };
        ...
        fn ensure_keymap(&mut self) -> Result<&xkb::Keymap> {
            ...
            Ok(self.keymap.as_ref().ok_or_else(|| {
                Error::Other("internal: keymap missing after compilation".to_string())
            })?)
        }
        ...
        fn connect_without_compositor_degrades_gracefully() {
            ...
            match VirtualInput::connect() {
                ...
                Err(e) => assert!(
                    matches!(
                        e,
                        Error::Unsupported(_) | Error::ProtocolMissing(_) | Error::Other(_)
                    ),
