...
use handfast_protocol::{
    ...
    TYPE_BATTERY_REQUEST, TYPE_CLIPBOARD, TYPE_CLIPBOARD_CONNECT, TYPE_FINDMYPHONE,
    TYPE_IDENTITY, TYPE_MOUSEPAD, TYPE_MPRIS, TYPE_MPRIS_REQUEST, TYPE_NOTIFICATION,
    TYPE_NOTIFICATION_REQUEST, TYPE_PAIR, TYPE_PING, TYPE_RUNCOMMAND, TYPE_RUNCOMMAND_REQUEST,
    TYPE_SHARE, TYPE_SFTP, TYPE_SYSTEMVOLUME, TYPE_TELEPHONY,
};
...
async fn frames_exactly_at_max_len_are_accepted() {
    let mut pad = 0usize;
    let packet = loop {
        let candidate = Packet::new(TYPE_PING, serde_json::json!({ "pad": "A".repeat(pad) }));
        let encoded_len = serde_json::to_string(&candidate).unwrap().len();
        if encoded_len == MAX_PACKET_LEN {
            break candidate;
        }
        assert!(encoded_len < MAX_PACKET_LEN);
        pad += MAX_PACKET_LEN - encoded_len;
    };

    let payload = serde_json::to_vec(&packet).unwrap();
    assert_eq!(payload.len(), MAX_PACKET_LEN);
