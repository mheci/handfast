...
fn handle_event(app: &mut HandfastApp, event: ServerEvent) {
    match event {
        ...
        ServerEvent::DeviceFound { id, name } => {
            match app.devices.iter_mut().find(|device| device.id == id) {
                // Re-announcement: refresh the name, keep richer state.
                Some(card) => card.name = name,
                None => app.devices.push(DeviceCard::from_found(&id, &name)),
            }
        }
        ServerEvent::DeviceLost { id } => {
            ...
            let had_selection = app.selected_id.is_some();
            app.sync_selection();
            if had_selection && app.selected_id.is_none() {
...
mod tests {
    ...
    fn toggling_a_plugin_dispatches_and_flips_optimistically() {
        ...
        let _ = receiver.try_next(); // consume the ListPlugins command
