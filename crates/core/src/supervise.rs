...
struct Entry {
    ...
    latest_child: Arc<Mutex<Option<JoinHandle<Result<()>>>>>,
}
...
impl Supervisor {
        ...
        Fut: Future<Output = Result<()>> + Send + 'static,
    {
        ...
        let latest_child: Arc<Mutex<Option<JoinHandle<Result<()>>>>> = Arc::new(Mutex::new(None));
        ...
        if let Some(old) = previous {
            tracing::debug!(
                ...
                "superseding previous supervision entry"
            );
    ...
    Fut: Future<Output = Result<()>> + Send + 'static,
{
    ...
    while !inner.shutdown.load(Ordering::SeqCst) {
        ...
        {
            // Park the handle where shutdown_all can find it. JoinHandle is not
            // Clone, so the watcher re-takes it below; a concurrent teardown
            // may win that race, which simply ends this watcher.
            let child = tokio::spawn(factory());
            *lock(&latest_child) = Some(child);
        }
        let Some(child) = lock(&latest_child).take() else {
            // shutdown_all drained the slot between park and take.
            return;
        };
...
mod tests {
    ...
    async fn wait_for(mut check: impl FnMut() -> bool, label: &str) {
        // Generous ceiling: CI runners throttle right after big compiles.
    ...
    fn backoff_doubles_and_caps_at_thirty_seconds() {
        ...
        // 2^8 * 100 ms = 25.6 s still under the cap; the next doubling would
        // be 51.2 s, so it clamps to MAX_BACKOFF from attempt 10 onward.
